...
| 命令 | 验证范围 |
| --- | --- |
| `make test-form-body-native` | 真实 Foundation 缓存实现：来源规则、同源及动态目标绑定、窗口 scope、query 保真、一次性消费、取消、超时及过期 |
| `make test-form-body-webkit` | 真实 macOS WKWebView：submit/formdata 时序、字段保留、SPA 来源变化、取消/重复提交、不支持的表单、两个窗口同时提交 |
...
- `FormBodyStoreTests.m`：直接编译生产缓存实现，不使用缓存替身；验证配置只限定来源，实际提交时再将 body 与同源 action URL 和 method 精确绑定。
